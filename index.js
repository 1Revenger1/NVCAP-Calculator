...
function readRom() {
    ...
    let rom = fs_1.readFileSync(romFile);
    ...
    parsedEntries.forEach((entry, index, parsedEntries) => {
        if (entry.merged)
            return;
        /*
         ...
         */
        let similarDCBs = parsedEntries.filter((value, filterIndex) => (entry.bus == value.bus && filterIndex != index));
    ...
    filteredEntries.forEach((display, index) => {
...
async function dumpDCBEntries() {
    parsedEntries.forEach((dcbHead, i) => {
...
function listDisplaysAndNvcap() {
    ...
    filteredEntries.forEach((display, index) => {
        let output = chalk_1.default.blueBright(`(${index + 1})`);
    ...
    console.log();
    console.log("Heads");
    let headTVOut = chalk_1.default.blueBright("(TV) ") + "[";
    let head0Out = chalk_1.default.blueBright("(1)  ") + "[";
    let head1Out = chalk_1.default.blueBright("(2)  ") + "[";
    headTV.forEach((dcbEntry) => headTVOut += (dcbEntry + 1) + ",");
    head0.forEach((dcbEntry) => head0Out += (dcbEntry + 1) + ",");
    head1.forEach((dcbEntry) => head1Out += (dcbEntry + 1) + ",");
    // trim last comma
    if (headTV.length > 0)
...
function createHeadMask(displays) {
    let mask = 0;
    displays.forEach((displayIndex) => {
        let dcbEntries = filteredEntries[displayIndex].dcbEntries;
        dcbEntries.forEach((dcbIndex) => {
            mask |= (1 << dcbIndex);
        });
    });
    return mask;
...
async function main() {
    ...
    readRom();
    while (true) {
        let romExists = fs_1.existsSync(romFile);
        // console.clear();
        header();
        let output = chalk_1.default.cyan("(1) ") + "Choose VBIOS/ROM file\n";
        if (romExists) {
        ...
        if (result.toLowerCase().startsWith("q"))
            break;
        if (result.toLowerCase().startsWith("1"))
            await chooseROM();
        if (romExists) {
            if (result.toLowerCase().startsWith("2"))
                await dumpDCBEntries();
            if (result.toLowerCase().startsWith("3"))
                await drawNVCap();
        }
    }
    showGoodbye();
